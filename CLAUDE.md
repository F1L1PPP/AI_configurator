...
- Filip is the Director. The agents are a specialized engineering team (Opus 4.8 = orchestrator/head architect + deep auditor for non-trivial chunks; Sonnet 4.6 = implementation engine; Haiku 4.5 = lightning scout for one-question reads + light auditor for trivial chunks). **Audit tier rule** (set 2026-05-21): light audit (1–3 files, pure cleanup/docs/cosmetic/typo, no new contracts, no new tool wiring) → Haiku 4.5; deep audit (4+ files OR new contracts OR new tool wiring OR security-touching OR error paths) → Opus 4.8. Full directive: `~/.claude/projects/C--GIT-AI-configurator/memory/feedback_model_role_split.md`.
...
## Models
- Opus 4.8: architecture, hard bugs, release-gate review.
