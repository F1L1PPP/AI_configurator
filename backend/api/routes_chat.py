...
async def chat(req: ChatRequest) -> ChatResponse:
    ...
    #
    # Chunk A2 (2026-05-21): double-try structure so the flag is set in the
    # success path BEFORE the outer finally fires. Python evaluates finally
    # after the inner try/except block exits — flag must be set inside the
    # inner try to be visible to the outer finally.
    keep_sessions_for_approval = False
    try:
        try:
        ...
        # Success path: if the planner left an action awaiting human approval,
        # keep the Playwright session alive so /api/execute can reuse it.
        # The flag is set HERE (inside the inner try, before the outer finally)
        # so the outer finally reads the correct value.
        if _pending_approval(result.events) is not None:
            keep_sessions_for_approval = True
    finally:
        # Keep sessions alive when the planner left an action awaiting approval
        # — /api/execute will need the session_id stored in the action params.
        # Close in all other cases (success with no pending approval, or any
        # exception path). Idempotent + safe when no sessions exist.
        # Chunk A2 (2026-05-21): chunk A's unconditional close broke the
        # propose → approve → execute multi-turn flow on WebUI actions.
        if not keep_sessions_for_approval:
            close_all_sessions()
